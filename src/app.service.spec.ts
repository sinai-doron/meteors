...
describe('App service', () => {
  ...
  describe('Slice the data for pagination', () => {
    it('should return paged data based on a valid page number', () => {
      const data = Array(50).fill({}); // Assuming a Meteor object is just an empty object for simplicity
      const result = appService.sliceResponse(data, '2');

      expect(result.meteors.length).toBe(9);
      expect(result.totalPages).toBe(6);
      expect(result.pageNumber).toBe(2);
      expect(result.totalElements).toBe(50);
    });

    it('should default to the first page for invalid page numbers', () => {
      const data = Array(50).fill({});
      expect(() => {
        appService.sliceResponse(data, '-2');
      }).toThrow();
    });

    it('should return the last page if the page number exceeds total pages', () => {
      const data = Array(50).fill({});
      const result = appService.sliceResponse(data, '6');

      expect(result.meteors.length).toBe(5);
      expect(result.pageNumber).toBe(6);
    });

    it('should round down the fractional page numbers', () => {
      const data = Array(50).fill({});
      const result = appService.sliceResponse(data, '2.5');

      expect(result.meteors.length).toBe(9);
      expect(result.pageNumber).toBe(2);
    });

    it('should default to the first page for non-numeric page strings', () => {
      const data = Array(28).fill({});

      expect(() => {
        appService.sliceResponse(data, 'abc');
      }).toThrow();
    });

    it(`should return a partial last page if the data doesn't divide evenly by the limit`, () => {
      const data = Array(28).fill({});
      const result = appService.sliceResponse(data, '4');

      expect(result.meteors.length).toBe(1);
      expect(result.pageNumber).toBe(4);
      expect(result.totalElements).toBe(28);
      expect(result.totalPages).toBe(4);
    });

    it('should handle an empty data array gracefully', () => {
      const data: Array<any> = [];
      const result = appService.sliceResponse(data, '1');

      expect(result.meteors.length).toBe(0);
      expect(result.pageNumber).toBe(1); // even with no data, it's the first page
      expect(result.totalElements).toBe(0);
      expect(result.totalPages).toBe(0);
    });

    it(`should handle data length that's exactly divisible by the limit`, () => {
      const data = Array(27).fill({});
      const result = appService.sliceResponse(data, '3');
